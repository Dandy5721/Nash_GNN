...
def main():
    ...
    if args.dataset.lower() in ['cora','citeseer',"computers","pubmed","texas","wisconsin","actor","squirrel","chameleon","cornell"] or args.dataset.lower().startswith("grb-"):
    ...
    if args.model.lower() == "sage":
        ...
        model = GraphCON_GCN(nfeat=data.num_features,nhid=args.hidden_channels,nclass=num_classes,
                             dropout=args.dropout,nlayers=args.num_layers,dt=1.,
                             alpha=0,gamma=0,res_version=2).to(device)
