...
#             # print(x.shape)
#             # x = x.squeeze()
#             # size_tuple = adj_t.size(0)
#             # print("Size (shape) of SparseTensor:", size_tuple)
#             # print(adj_t.shape)
#             # adj_t = adj_t.squeeze()
#             # x = x.float()
#             # adj_t = adj_t.float()
...
class SAGE(nn.Module):
    def __init__(self, dataset, hidden, num_layers, dropout, layer_norm_first=False, use_ln=True):
...
class GraphCON_GCN(nn.Module):
    ...
    def forward(self, data, adj):
        input = data.float()
        edge_index, values = to_edge_index(adj)
