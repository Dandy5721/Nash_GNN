...
class ODEFuncTransformerAtt(ODEFunc):
  ...
  def __init__(self, in_features, out_features, adj):
    ...
    # self.edge_index1 = adj.to(device).indices()
    # self.edge_index1 = adj._indices()
    # self.edge_attr = adj._values()
...
class MEANCURV(nn.Module):
    def __init__(self, dataset, hidden_features, n_layers, activation=F.relu, layer_norm=False, residual=False, feat_norm=None, adj_norm_func=GCNAdjNorm, dropout=0.0):
        super(MEANCURV, self).__init__()
        self.in_features = dataset.num_features
        ...
        n_features = [self.in_features] + hidden_features + [self.out_features]

        self.layers = nn.ModuleList()
        for i in range(n_layers):
            if layer_norm:
                self.layers.append(nn.LayerNorm(n_features[i]))
            self.layers.append(GCNConv(n_features[i], n_features[i + 1]))
...
class GCNConv(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        """
        x = self.linear(x)
        # adj = torch.sparse_coo_tensor(adj._indices(), adj._values(), adj.size)
