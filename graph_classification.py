...
from asap_pool_model import ASAP_Pool, GCN, GAT
from models.model_tudataset import *
from hang_model.GNN_graphcon_tudataset import *
...
class Trainer(object):
    ...
    def __init__(self, params):
        ...
        if self.p.gpu != '-1' and torch.cuda.is_available():
            gpu_id = int(self.p.gpu)  # Make sure `self.p.gpu` is the ID of the GPU you want to use
            self.device = torch.device(f'cuda:{gpu_id}')
            torch.cuda.set_device(self.device)  # Set the device for PyTorch operations
    ...
    def onedegree(self, data ) :
        assert data.edge_index is not None
        idx, x = data.edge_index[0], data.x
        deg = degree(idx, data.num_nodes, dtype=torch.long)
        deg = one_hot(deg, num_classes=max(deg) + 1)
    ...
    def loadData(self):
        dataset_path = osp.join(
            '**/data',
            self.p.dataset.lower(),
            self.p.dataset.upper(),
            'raw'
        )
        # transform = T.Compose([T.ToUndirected(), T.AddSelfLoops()])
        # transform = T.Compose([T.ToSparseTensor()])
        # transform = T.OneHotDegree(max_degree=1) 
        dataset = TUDataset(dataset_path, self.p.dataset.upper())
        max_degree = 0
        for data in dataset:
            degrees = data.edge_index[0].bincount()
            max_degree = max(max_degree, degrees.max().item())
        transform = T.OneHotDegree(max_degree=max_degree) 
#         transform = T.Compose([
#     T.Constant(),
#     T.ToSparseTensor()  # Optional, if you need the adjacency matrix in sparse format
# ])
        dataset = TUDataset(dataset_path, self.p.dataset.upper(), use_node_attr=self.p.use_node_attr,transform=transform)
        print(dataset[0])
    ...
    def addModel(self):
        ...
        elif self.p.model == 'GAT':
            model = GAT(
                dataset=self.data,
                hidden=self.p.hid_dim,
            )
        elif self.p.model == 'SAGE':
            model = SAGE(
                dataset=self.data,
                hidden=self.p.hid_dim,
                num_layers=self.p.num_layers,
                dropout=self.p.dropout_att,
            )
        elif self.p.model == 'ICNN':
            opt = {
    'hidden_dim': self.p.hid_dim,
    # 'num_nodes': self.data.num_nodes,
    'num_classes': self.data.num_classes,  # Corrected from num_features to num_classes
    'input_dropout': 0.0,
    'dropout': self.p.dropout_att,
    'function': self.p.function,
    'fc_out': self.p.fc_out,
    'time':self.p.time,
    'use_mlp':self.p.use_mlp,
}
            dataset=self.data.num_features,
            # opt['hidden_dim'] = self.p.hid_dim
            # opt['num_nodes'] = self.data.num_nodes
            # opt['num_classes'] = self.data.num_features
            # opt['input_dropout'] = 0.0
            # opt['dropout'] = self.p.dropout_att
            # opt['function'] = self.p.function
            # opt['fc_out'] = self.p.fc_out
            model = GNNGraphCon(opt, dataset, self.device)
...
if __name__ == '__main__':
    ...
    num_seeds = 5  # Number of seeds you want
    seeds = [0]
